...
export default function IronTriangleStats() {
  return (
    <section style={{ background:"#001e3c", padding:"clamp(3rem,5vw,5rem) clamp(1.2rem,4vw,2rem)", position:"relative", overflow:"hidden" }}>
      ...
      <style>{`
        @media(max-width:900px){.triangle-grid{grid-template-columns:1fr 1fr!important}.quote-grid{grid-template-columns:1fr!important}} @media(max-width:600px){.triangle-grid{grid-template-columns:1fr!important}}
      `}</style>
    </section>
  );
}
